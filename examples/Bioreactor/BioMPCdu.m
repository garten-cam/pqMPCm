...
dr_rfs = [0.0, 0.273, 0.010, 0.273, 0.09, 0.27, 0.15];
...
%
% Calculate the error
st_err = arrayfun(@(dcp)dcp.abs_error(expts),dcps);
% whos the best
...
xlabel('$t$ [h]',Interpreter='latex')
ylabel('$D$',Interpreter='latex')
legend('Dulution rate',Interpreter="latex",Location="southeast")
set(gcf,'PaperPosition', [0, 0, 16, 8])
% saveas(st_fig,"~/Documents/BioReactorConf/figures/pqEDMD_fast_low.fig")
% saveas(st_fig,"~/Documents/BioReactorConf/figures/pqEDMD_fast_low.eps", "epsc")
%%
%-------------------------------------------------------------------------%
%---------- Control ------------------------------------------------------%
%-------------------------------------------------------------------------%
...
mpc_mat = pqMPCdu_mat(dcp, ... Decompostition objedct
	Np, ... Np
	Nc, ... Nc
	[2 0; 0 0], ... Qy only interested in biomass
	.5*eye(dcp.m), ... Qu 
...
ctr_sim.y(1,:) = [(0.1-0.05)*rand(1, 1)+0.05, (0.2-0.01)*rand(1, 1)+0.01];
...
for step = 2 : ctr_points
	...
	ctr_sim.u(step-1,:) = pqMPCdu(ctr_sim.u(step-1,:), ... The input form the last step
		ctr_sim.y(step-1,:), ... Output from last iteration
		y_ref, ... reference output for this sim step
		dcp, ... 
		mpc_mat, ...
		[0.01 4] ... vector of delta[u_min u_max]
		);
...
set(gcf,'PaperPosition', [0, 0, 16, 8])
% saveas(ctr_fig,"~/Documents/BioReactorConf/figures/ctr_fast_low.fig")
% saveas(ctr_fig,"~/Documents/BioReactorConf/figures/ctr_fast_low.eps", "epsc")
